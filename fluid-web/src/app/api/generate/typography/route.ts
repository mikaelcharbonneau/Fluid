...
// POST /api/generate/typography — recommend a type pairing for one of the
...
export async function POST(request: Request) {
  ...
  if (!String(brand.brief ?? "").trim()) {
    return NextResponse.json(
      { error: "Add a brief before generating typography." },
      { status: 400 },
    );
  }
  ...
  let typography;
  try {
    typography = await generateBrandTypography({
    ...
    const message =
      err instanceof Error ? err.message : "Typography generation failed.";
    return NextResponse.json({ error: message }, { status: 502 });
  }

  const nextData = { ...(brand.data as Record<string, unknown>), typography };
